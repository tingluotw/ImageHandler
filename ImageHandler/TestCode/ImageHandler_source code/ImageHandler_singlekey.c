 ...
 *   Description:
 *      0. Get ZwCreateProcess native api's address and send it to kernel driver
 *		1. Receive the captured image path from kernel driver
 *      2. Send it to VirusTotal and get scan_id field
 *      3. Ask report periodically, and get positive field
 *      4. Check the result, and tell the driver to execute the image or not
 *************************************************************************************************************/
...
#define SCAN_PAUSE 59000								/* microsecond */
#define REPORT_PAUSE 31000
#define DONE_PAUSE 2000
...
{
	//StringCchCopy(pipe->chReply, BUFSIZE, chreply);
	//_tprintf(TEXT("[%x] %d\n"), pipe->hPipeInst, pipe->reply);
	//pipe->cbToWrite = (lstrlen(chreply) + 1)*sizeof(TCHAR);
...
CHAR* CheckForImagePath(CHAR* imagepath)
{
	...
	else if (strncmp(imagepath, str2, strlen(str3)) == 0){
		ret = strrchr(imagepath, '?');
		ret = ret + 2;
	}
	else
...
INT MoveObjToReportQueue(FileState* node)
{
	...
	case WAIT_OBJECT_0:
		...
		__finally{
			...
			{
				printf(">>Error: MoveObjToReportQueue: Release mutex_rq error\n");
				ret = -1;
			}
		}
		break;
	case WAIT_ABANDONED:
		printf(">>Error: MoveObjToReportQueue: Wait abandoned\n");
		ret = -1;
		break;
	}//end of switch
			...
			if (file->id > 0)
			{
				...
				if (ret != 0)
				{
					...
					{					
						...
						printf(">> Error: Scan file error! (%d times) %s\n", file->scan_num, file->filepath);
			...
			if (file->positive < 0)
			{
				...
				if (ret != 0)
				{
					...
					{						
						...
						printf(">> Error: report file error!key:%d (%d times) %s \n", file->key_index, file->report_num, file->filepath);
