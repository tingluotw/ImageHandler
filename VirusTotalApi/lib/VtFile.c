...
int VtFile_scan(struct VtFile *file_scan, const char *file_path, const char *notify_url) {
  ...
  DWORD currentpath_size = 128;
  TCHAR currentpath[512];
 // TCHAR certpath[] = TEXT("\\ca-bundle.crt") ;
  ...
  ret = GetCurrentDirectory(currentpath_size, currentpath);
  if (ret == 0)
  {
 ...
 //curl_easy_setopt(curl, CURLOPT_CAINFO, "C:\\MyVtDll\\Debug\\ca-bundle.crt");
  //curl_easy_setopt(curl, CURLOPT_CAINFO, _tcscat_s(currentpath, sizeof(TCHAR)*(_tcslen(certpath)+1), certpath));
  curl_easy_setopt(curl, CURLOPT_CAINFO, ".\\ca-bundle.crt");
 //printf("cert path is: %ws", currentpath);
...
int VtFile_rescanDelete(struct VtFile *file_scan,
  ...
  /* Check for errors */
  if(res != CURLE_OK) {
    VT_ERROR("curl_easy_perform() failed: %s, %d\n", curl_easy_strerror(res), res);
    ret = res;
    goto cleanup;
  } else {
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &http_response_code);
...
int VtFile_report(struct VtFile *file_scan, const char *hash) {
  ...
  CHAR certpath[] = ".\\ca-bundle.crt";
  CHAR error[1024];

  //if (GetCurrentDirectory(256, current_di))
  ...
  curl_easy_setopt(curl, CURLOPT_VERBOSE, 1);
  //curl_easy_setopt(curl, CURLOPT_PROXY, "140.115.1.39:3128");
 ...
 // curl_easy_setopt(curl, CURLOPT_PORT, 80);
  ...
  /* Perform the request, res will get the return code */
  printf(">>>before curl_easy_perform: api_key=%s \n", file_scan->api_key);
  printf(">>>before curl_easy_perform: scan_id=%s \n", hash);
  res = curl_easy_perform(curl);
  DBG(1, "Perform done\n");
  /* Check for errors */
  if(res != CURLE_OK) {
